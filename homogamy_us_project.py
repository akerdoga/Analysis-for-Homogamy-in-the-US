...
#Learn what is the number of rows and columns in data to get to know the dataset better before dwell into it.
woman_data.shape
...
# As we can see above, the minimum year of education for women (educw) is 5 and maximum is 17. The minimum year of education for men (educh) is 3 and maximum is 17.

# #### Correlation Heat Map
# I aim to understand if there is a correlation between which factors are correlated with the education level of women and education level of men. It is necessary to see the correlated factors so that I can select the instrumental variables and which variables I should control in regression.  
# 
...
correlation_matrix = woman_data.corr()
...
sns.heatmap(correlation_matrix, mask=mask, cmap=cmap, annot=True, fmt=".2f", center=0, square=True, linewidths=0.5, cbar_kws={"shrink": 0.8})
...
# As we can see on the heatmap, variables that are correlated with educw and educh are educwf, educwm, income and wageh. It means that these values will be controlled in the regression analysis to control for endogeniety problem. 

# #### Classifying Educational Attainment
# 
# There are 5 levels of education in the US in 1970s. The education levels and the years required for attainment are as follows: 
...
# The histogram shows us that education levels in the US of spouses are mostly the same. Homogamy is present!
...
plt.xticks(range(5))
plt.xlabel('Difference in Education Level')
plt.ylabel('Number of Couples in the City')
...
plt.xticks(range(5))
plt.xlabel('Difference in Education Level')
plt.ylabel('Number of Couples Not in the City')
...
# Lastly, I plot a pie chart to visualize the distribution of educational levels for couples with educational homogamy
...
# I created a variable namely 'homogamy', which is a binary variable that says True for educational difference 0, and else False.
